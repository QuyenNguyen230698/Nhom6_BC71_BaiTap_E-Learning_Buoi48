    ...
    return (
        <div className='py-20 flex flex-col gap-6 w-full h-full bg-home'>
            <div className='w-full flex flex-wrap items-center container mx-auto gap-2'>
                ...
                {showPersonalInfo ? (<div className="join items-center justify-center">
                    <input onChange={autoSearchUser} className="input input-bordered join-item text-gray-400" placeholder="Search" />
                    <button onClick={clearInput} className="btn join-item text-gray-400">Clear</button>
                </div>) : (<div className="join items-center justify-center">
                    <input onChange={autoSearch} className="input input-bordered join-item text-gray-400" placeholder="Search" />
                    <button onClick={clearInput} className="btn join-item text-gray-400">Clear</button>
                </div>) }
            </div>
            {showPersonalInfo ? (
               <AccountListUser searchuser={searchuser} currentItems={currentItems} indexOfFirstItem={indexOfFirstItem} handleEditUserClick={handleEditUserClick} handleDeleteUsers={handleDeleteUsers}/>
            ):(<AccountListCourse searchuser={searchuser} onButtonClick={reloadCourseList} listCourse={listCourse} previewImage={previewImage}/>)}
